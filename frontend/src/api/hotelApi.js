...
export const createHotelOwner = async (token) => {
    return await axios.post("http://localhost:8080/api/v1/hotel-owner/create-owner", {}, {
        headers: { Authorization: `Bearer ${token}` }
    });
};

export const registerHotel = async (token, hotelData) => {
    return await axios.post("http://localhost:8080/api/v1/hotel-owner/register-hotel", hotelData, {
        headers: { Authorization: `Bearer ${token}` }
    });
};

export const addRoom = async (token, hotelId, roomData) => {
    return await axios.post(`http://localhost:8080/api/v1/rooms/hotel/${hotelId}`, roomData, {
        headers: { Authorization: `Bearer ${token}` }
    });
};

export const uploadHotelMedia = async (token, hotelId, formData) => {
    return await axios.post(`http://localhost:8080/api/v1/hotel-media/${hotelId}`, formData, {
        headers: {
...
export const uploadHotelDocument = async (token, documentType, file) => {
    ...
    return await axios.post("http://localhost:8080/api/v1/document/upload", formData, {
        headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "multipart/form-data"
        }
...
export const searchHotels = async (token, params) => {
    ...
    return await axios.get("http://localhost:8080/api/v1/hotels/search", {
        params,
        headers: { Authorization: `Bearer ${token}` },
...
export const bookHotel = async (token, bookingData) => {
    return await axios.post("http://localhost:8080/api/bookings/book", bookingData, {
        headers: { Authorization: `Bearer ${token}` }
    });
};
