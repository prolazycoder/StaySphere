...
public class UserService {
    ...
    public Map<String, Object> refreshAccessToken(String refreshToken) {
        ...
        Optional<UserLogin> loginOptional =
                userLoginRepository.findByRefreshToken(refreshToken);
        if (loginOptional.isEmpty()) {
            response.put("success", false);
            response.put("message", "Refresh token not found");
            return response;
        }
        UserLogin login = loginOptional.get();
        if (login.getRevoked()) {
            response.put("success", false);
            response.put("message", "Refresh token revoked");
            return response;
        }

        if (login.getRefreshTokenExpiry().isBefore(LocalDateTime.now())) {
            response.put("success", false);
        ...
        String newAccessToken = jwtUtil.createNewAccessToken(refreshToken, role);
        String newRefreshToken =
                jwtUtil.generateRefreshToken(email, userId, UserRole.valueOf(role));

        redisService.save(newAccessToken, userId.toString(), jwtUtil.getTokenValidity());

        // Update DB
        login.setRefreshToken(newRefreshToken);
        login.setRefreshTokenCreatedAt(LocalDateTime.now());
        login.setRefreshTokenExpiry(LocalDateTime.now().plusDays(7));
        login.setRevoked(false);
        ...
        response.put("message", "New tokens generated");
