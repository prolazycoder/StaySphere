...
public class HotelService {
    ...
    public Hotels registerHotel(String authHeader, HotelCreateRequest hotelRequest) {
        ...
        Hotels hotel = Hotels.builder()
                ...
                .ownerId(owner.getHotelOwnerId().toString())
                .images(List.of())
                .videos(List.of())
                .build();
    ...
    public List<Hotels> searchHotels(
        ...
        if (city != null && !city.isEmpty()) {
            query.addCriteria(Criteria.where("city").regex(Pattern.quote(city), "i")
            );
        }
        if (state != null && !state.isEmpty()) {
            query.addCriteria(Criteria.where("state").regex(Pattern.quote(state), "i")
            );
        }
        if (country != null && !country.isEmpty()) {
            query.addCriteria(Criteria.where("country").regex(Pattern.quote(country), "i")
            );
        }
    ...
    @Transactional
    public ResponseEntity<Map<String, Object>> createHotelOwnerProfile(String authHeader) {
        ...
        if (hotelOwnerRepository.findByUserId(userId).isPresent()) {
            throw new RuntimeException("Hotel owner profile already exists");
        }
        HotelOwner owner = HotelOwner.builder()
                .user(user)
                .panVerified(false)
                .gstVerified(false)
        ...
        Map<String, Object> response = new HashMap<>();
        response.put("message", "Hotel owner profile created successfully");
        response.put("hotelOwnerId", owner.getHotelOwnerId());
